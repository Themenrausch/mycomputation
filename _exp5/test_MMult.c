...
double dclock();
void cblasversion(int, int, int, double *, int, double *, int, double *, int);
void OMP_MMult(int, int, int, double *, int, double *, int, double *, int);
void thread_gemm(int , int , int , double *, int , double *, int , double *, int);
...
// #pragma omp parallel for
  for (p = PFIRST; p <= PLAST; p = 2 * p)
  {
    ...
    dtime_c = dclock();
    cblasversion(m, n, k, a, lda, b, ldb, cref, ldc);
    dtime_c = dclock() - dtime_c;
    ...
    for (rep = 0; rep < NREPEATS; rep++)
    {
      ...
      // OMP_MMult(m, n, k, a, lda, b, ldb, c, ldc);
      thread_gemm(m, n, k, a, lda, b, ldb, c, ldc);
      dtime = dclock() - dtime;
    ...
    printf("%d %le %le %le \n", p, gflops / dtime_best, gflops / dtime_c, diff);
    fflush(stdout);
